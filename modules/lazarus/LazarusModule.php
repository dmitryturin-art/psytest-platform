...
final class LazarusModule extends BaseTestModule
{
    ...
    private function aiPairContext(array $comparison): ?array
    {
        ...
        return [
            ...
            'items' => $items,
            // Голое число модель прочитать не может: в первом живом разборе она
            // честно написала, что определение показателя ей не передали, и
            // отказалась делать из него выводы. Поэтому значение идёт вместе
            // с определением — тем же, что видит человек на странице результата.
            'agreement' => [
                'percent' => $comparison['overall_agreement'] ?? null,
                'means' => 'совпадение собственных оценок отношений: насколько близки баллы, '
                    . 'которые партнёры поставили сами за себя. 100 % — оценки совпали по всем '
                    . '16 пунктам; каждый балл расхождения внутри пункта снижает показатель. '
                    . 'Ожидаемые оценки партнёра в этот показатель не входят.',
            ],
            'partner1' => $partner1,
            'partner2' => $partner2,
        ];
