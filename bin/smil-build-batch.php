 ...
 *
 * Партии задаются списком BATCHES: 05.S3.1 (16 записей) и 05.S3.2 (20 клинических
 * подшкал), обе утверждены владельцем 15.09.2026. Ключи и нормы НЕ перепечатываются
 ...
 * 05.S3.2 — вторая партия, 20 клинических подшкал.
 */
const BATCHES = [
    '05.S3.1' => [
        1 => 'A',
...
/**
 * Записи, включённые решением владельца со статусом verified-with-note.
 *
 * Номер записи => причина. Текст попадает в поле note runtime-файла и показывается
 * в результате и PDF рядом с пометкой «нормы требуют сверки».
 */
const NOTES = [
    72 => 'В источнике нормы записи №72 совпадают с нормами записи №74 («Контроль над '
        . 'враждебностью») — вероятная опечатка печатного приложения. Значения перенесены '
        . 'как напечатано.',
    87 => 'В источнике вторая строка ключа напечатана «11 верно» вместо «11 неверно»; '
        . 'оба прохода транскрипции прочитали её как список «неверно», так она и перенесена.',
];
...
foreach (BATCHES as $batch => $codes) {
    ...
    foreach ($codes as $number => $code) {
        ...
        $scale = [
            ...
            'code' => $code,
            'name' => $entry['name'],
            'status' => isset(NOTES[$number]) ? 'verified-with-note' : 'verified',
            'batch' => $batch,
            'source' => [
...
$output = [
    ...
    'note' => 'Партия 05.S3.1: 16 записей со статусом verified. Партия 05.S3.2: 20 клинических подшкал, '
        . 'из них 18 verified и две (№72, №87) verified-with-note — причина в поле note шкалы. '
