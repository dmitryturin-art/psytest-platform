...
- Не сделано / риски: **вопрос владельцу** — включён ли APCu на Beget? Если да, то до отдельного пакета остаётся окно: реестр с живыми инстансами живёт в кэше час, а сериализованный `$modulePath` указывает в каталог предыдущего релиза, поэтому после выкладки `questions.json`/`metadata.json` могут до часа читаться из старого релиза. `isUsableRegistry()` закрывает порчу объектов, но не устаревание путей. Если APCu выключен — вопрос снимается, ветка кэша мертва.
- Следующий шаг: ответ владельца по APCu на Beget → при `enabled` пакет «кэшировать карту модулей вместо инстансов».
