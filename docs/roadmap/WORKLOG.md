...
- Причина (владелец 24.09): двойное нажатие «Создать ссылку» создало два одинаковых приглашения (redirect после POST есть, кнопка не блокировалась, CSRF-токен сессионный); из формы приглашения нельзя было завести нового клиента — пришлось отправлять без карточки.
- Сделано: `core/FormOnce.php` — одноразовый ключ формы `form_key` (в сессии до 20 ключей, TTL 12 ч, с результатом первого ответа): повторный POST с тем же ключом ничего не создаёт и возвращает тот же flash со ссылкой; без ключа/с чужим — ошибка без побочных эффектов; ошибка валидации возвращает ключ в оборот. Файловая блокировка сессии PHP гарантирует, что второй запрос двойного клика видит результат первого. Клиент: `public/js/owner-forms.js` блокирует кнопку («Создаём…»), без JS работает сервер. `core/OwnerInviteSubmission.php` — обработка формы вынесена из контроллера; пункт «Новый клиент…» (`__new__`) с полем имени (обязательно в этом режиме), валидация через общий `TherapistClientService::isValidInput`, карточка и приглашение в одной транзакции. Helper готов для «Заказать разбор» и «Создать клиента» (не подключён — отдельный пакет).
- Проверки и evidence: исполнитель — полный `composer test` 562 tests / 48 892 assertions OK (+4 unit FormOnce, +6 integration, +1 контракт шаблона); analyse OK, baseline 141/141; lint 0/174; архитектура OK; браузер — два одновременных POST с одним ключом → одно приглашение; двойной клик → одно; «Новый клиент…» на 390×844 создаёт карточку и приглашение с привязкой, console errors 0. Ведущий: `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден**.
- Попутно замечено: в мобильной шапке кабинета «Выйти» переносится на вторую строку (доэтапное, к пакету дизайна).
- Следующий шаг: выкладка по сигналу владельца; WP8 (compact vs full) — владелец переключает режим сам либо через Claude in Chrome.
...
- Цель: 30 записей — всё, что оставалось в приложении сверх отложенных (№7, 19, 22, 23, 38, 47, 50, 52, 55, 56, 64, 70, 74, 81, 85, 88, 94, 95, 99, 122, 135, 162, 172, 181, 205, 206, 209, 210, 211, 212); состав задан ведущим по сигналу владельца «начинай S3.5» 15.09 (№56 добавлена после замечания исполнителя); глоссарий утверждён владельцем 15.09 с двумя правками.
- Сделано: `additional-scales-v2.json` — 105 шкал (партии 1–4 идентичны запись-в-запись, эталоны прежних 75 колонок без изменений); NAME_OVERRIDES: №47/№52 «Преступность (I)/(II)», №50 «Доминирование (II)», №56 «Скрытая депрессия» (в источнике «Мягкая депрессия» — неточный перевод subtle; по аналогии с парами «явная/скрытая» приложения; решение владельца 15.09); NOTES у 10 записей (в т.ч. №74 — нормы совпадают с исключённой №72; №162/№205 — зеркальная пара Gough 1951; №52/№55/№56 — сжатый кегль). Сверка: подтверждены Ca (Williams 1952, 36 — русское название «хвостатое ядро» — ошибка перевода caudality), Gough Ds-r (40), Hy2 (12), Hy5 (7), Gough Pr (32), Tydlaska–Mengel Wa (37), D-Subtle (20 — **№51 ∪ №56 = ровно 60 пунктов 2-й шкалы**, это же разрешило спорную цифру 160/166 в ключе №56, закреплено тестом); вероятны 7; не установлены/не подтверждены 15. Глоссарий 105/105 по правилам G5 (обе трактовки одной фразой у №52, 55, 81, 94, 205; трактовка по полу у №122; медицинские/криминологические названия — «исследовательская шкала, не диагноз»); описание Собчик с печ. с. 139 переставлено к №50; №122 «Женственность интересов» разведена с №66. По замечанию владельца 15.09 («не сглаживай терминологию, будь как клинический психолог») №38 «Адвокатский тип личности» описана жёстче: пересказ Собчик (социальная гибкость, потребность в признании, проповедование морали, мягкость) + компиляция Cs «службизм» (дисциплинированность, регламентация; полюс — «богемный» уклад), высокие — конформно-примирительная позиция, вытеснение агрессии. Пороги AI-контекста: full 64 000 → 95 000 (факт 90 614), compact 45 000 → 65 000 (факт 62 216, 0,687 × full). Реестр, сверка, таблица G7 в `docs/smil-glossary-verification.md`, README модуля, AUDIT_TRACEABILITY 105/113.
- Проверки и evidence: исполнитель — полный `composer test` 551 tests / 48 823 assertions OK; analyse OK, baseline 141/141; lint 0/170; архитектура OK; `--check` 105; браузер — 105 строк, note у 10 записей, PDF 200 (88 КБ), console errors 0, на 390 px известный доэтапный дефект таблиц не ухудшен. Ведущий — `test:fast` 435 OK после правок; `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден** (дважды: до и после правок владельца).
- Не вошли (8/113): №9, 58, 72, 167 ждут другого издания; №175, 176, 178, 179 — шкалы только для одного пола, партия S3.6 с поддержкой в калькуляторе. **Полный режим глоссария дальше наращивать некуда** (запас ≈4,8 %): S3.6 — на компактном режиме по умолчанию либо с сокращёнными полями `source`.
- Следующий шаг: выкладка по сигналу владельца; S3.6; результат WP8 (full vs compact) от владельца.
...
- Цель (вопрос владельца 15.09: не перегружает ли модель полный глоссарий 75 шкал; решение — режим переключателем для сравнения WP8 на одном кейсе): по шкалам в диапазоне 40–65T передавать только краткое значение, полные пояснения — по шкалам вне диапазона.
- Сделано: `core/Ai/SmilGlossaryCompactor.php` — чистая функция поверх готовой нагрузки, применяется в `AiReportContextBuilder` и `PromptFixtureContext` (предпросмотр совпадает с боевым запросом, проверено тестом); `SmilModule` о настройке не знает (граница PRODUCT_RULES §6). Настройка `smil_glossary_mode` (`full` по умолчанию / `compact`) в `ai_settings` через `AiSettings`, мусорное значение = `full`; в контексте всегда поле `glossary_mode` (видно в `context_snapshot`), в compact — фраза в `levels.principles` о кратких записях; шкала без T-балла сохраняет полную запись; `additional_scales`, `additional_scales_without_glossary`, `levels.bands` не меняются. Кабинет «Промпты»: радио «Глоссарий СМИЛ: полный / компактный», предпросмотр показывает режим и длину контекста в знаках. ARCHITECTURE.md, README модуля.
- Замер на эталонном профиле контракта: full 60 315 знаков, compact 43 211 (−28 %; полная запись у 38 шкал, краткая у 37 — синтетический профиль разбрасывает T от 20 до 77). Порог compact в контракте 45 000 и compact < 0,8 × full; ориентир ≤ 35 000 на этом профиле недостижим — на живых профилях средних шкал больше и экономия выше. Порог full 64 000 не тронут.
- Проверки и evidence: исполнитель — полный `composer test` 548 tests / 39 202 assertions OK (+14); analyse OK, baseline 141/141; lint 0/170; архитектура OK; браузер — переключение режима в «Промптах», предпросмотр 60 315 → 43 211 знаков, console errors 0, 390×844 без горизонтального скролла. Ведущий: `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден**.
- Следующий шаг: выкладка по сигналу владельца; владелец переключает режим и перезаказывает разборы по кейсу «Дмитрий» (WP8: full vs compact); S3.5.
...
- Цель (D-055, замечание владельца 15.09 «это не игрушки»): перепроверить значения и интерпретации всех 75 шкал по библиотеке владельца, западной литературе и русскоязычным сайтам.
- Источники: «MMPI. Общие сведения.pdf» (компиляция без авторства, ссылается на Собчик 2000; разд. «Дополнительные шкалы», печ. с. 17–27 — **58** шкал с кодами и описаниями), Собчик 2003 (печ. с. 137–140, ~26 кратких характеристик), Соломин, Березин–Мирошников, западные первоисточники (Harris–Lingoes, Wiener–Harmon, Edwards, Gough, Cuadra, Pepper–Strong), русскоязычные сайты (psylab.info, psytests.org, studbooks.net — нового не дали, помечены как вторичные, ссылки с датой обращения в документе).
- Результат: `docs/smil-glossary-verification.md` — таблица по 75 шкалам; статусы: совпадает 26, уточнено 16, противоречие 5, нет источника 28. Изменено 25 записей глоссария (14 — тексты, 11 — только `source`): №36 Соперничество, №106 Лидерство, №177 Играние роли (были «по названию» — теперь по Собчик/компиляции), №193 Реакция соматизации (второе название «конформизм»; телесная природа только при высоких 1-й и 2-й), №200 Социальный статус (тяга к людям, гражданственность), №26 Контроль (высокие = сверхконтроль), №121 Альтруизм (явная трактовка по полу из Собчик), точечно №2, 93, 110, 119, 141, 189, 196. Омонимы описаны (Cp/Cr, Es №62/№92, Ec vs SAL, St-r/Ss, №47/№52, №49/№50).
- Решения владельца 15.09 (внесены `5c805e7`): обе трактовки в одну карточку только для пяти спорных шкал, одной фразой; ведущая трактовка — №6 Собчик, №84 западная (по составу пунктов), **№90 «Общие сведения» (тенденция перечить, нонконформизм)**, №148 западная (направление в компиляции ошибочно), №194 Edwards; компиляция — вторичный источник; числа компиляции (MF в процентах) не переносить; трактовка по полу остаётся на уровне норм, в тексте только где источник явно говорит; в `levels.principles` добавлено предупреждение о ревалидизации (Соломин, Березин–Мирошников); у Do пометка, что описание Собчик стоит при №50; №122 «Женственность интересов» — отдельной шкалой в S3.5.
- Проверки: исполнитель — `test:fast` 422 OK, `tests/Smil` 51 OK, `AiReportContextContractTest` 23 OK, analyse/lint OK. Ведущий после правок — `test:fast` 422 OK, lint OK, analyse OK; длина AI-контекста СМИЛ 60 292 знака — порог 60 000 → **64 000** (дальнейший рост закрывать компактным режимом, не порогом); `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден**.
- Следующий шаг: merge/выкладка по сигналу владельца; решение о компактном глоссарии (полные пояснения только вне 40–65T) и WP8; S3.5 с №122.
...
- Цель: партия из 20 записей без аномалий сверки (№26, 36, 59, 66, 106, 109, 110, 111, 114, 115, 119, 121, 169, 177, 189, 194, 196, 200, 203, 208), состав задан ведущим по сигналу владельца «начинай S3.4» 15.09; глоссарий принят владельцем 15.09 **предварительно** (см. D-055).
- Сделано: `additional-scales-v2.json` — 75 шкал (партии 1–3 идентичны запись-в-запись, эталоны прежних 55 колонок не изменились ни в одном наборе); коды Harris–Lingoes Ma1/Ma2, пары MaO/MaS; №177 в runtime «Играние роли» (`NAME_OVERRIDES`, написание источника «Играния роли» в `note`); эталоны 8 наборов × 75, инварианты, `additional_scales_count` 75; сверка (`docs/smil-western-crosscheck.md`): №114 и №115 не пересекаются и в объединении дают ровно 46 пунктов 9-й шкалы, Ma1/Ma2 вложены в 9-ю, №26 Cn Cuadra (50), №194 Edwards SD первой редакции (79), №119 Pepper–Strong Ego-Sensitivity (15); **№200 «Социальный статус» — прототип Gough St отвергнут (34 п. против 19)**, шкала включена и трактуется по названию и ключу; у 11 записей прототип не установлен (Dahlstrom–Welsh 1975 в открытом доступе не индексируется). Глоссарий 75/75, `additional_scales_without_glossary` пуст; тексты 11 шкал без прототипа написаны только по названию и составу ключа; SDS/TTD описаны как установка при заполнении. Длина AI-контекста СМИЛ ≈56,1 тыс. знаков (было ≈40,5) — порог контракта 44 000 → 60 000 с обоснованием. Реестр, README модуля, AUDIT_TRACEABILITY (75/113).
- Проверки и evidence: исполнитель — полный `composer test` 534 tests / 39 076 assertions OK; `test:fast` 422 OK; analyse OK, baseline 141 (cap 141); lint 0/167; архитектура OK; `smil-build-batch.php --check` 75; golden базовых без diff; браузер — результат 200 с 75 строками, `note` у RPL виден, PDF 200 (65,7 КБ), console errors 0. Ведущий: `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден**.
- Найдено попутно (не исправлялось): на 390 px таблицы результата СМИЛ переполняют экран (`body.scrollWidth` 436 на `main` до партии, 457 после) — `.scores-table` без обёртки `overflow-x`, самый широкий элемент — строка-заголовок группы; переполняется и таблица базовых 13 шкал. Отдельный пакет вёрстки результата.
- Замечание владельца 15.09 (после приёмки): значения и интерпретации **всех** дополнительных шкал перепроверить по его библиотеке и открытым источникам — «это не игрушки»; сомнения по терминологии (например, «Женственность» №66 против «Женственность интересов» №122 — разные показатели, трактовка по полу). В библиотеке владельца найден источник с описаниями: «MMPI. Общие сведения.pdf» (стр. 17–27, 56 дополнительных шкал с кодами Ac…To и текстами: Cn «Контроль», Cp «Соперничество», Rp, Sp, St-r, Ma1 и др.), плюс руководство Соломина и Березин–Мирошников. Оформлено как D-055 и пакет 07.G5.
- Вопрос владельца 15.09 (открыт): не перегружает ли модель полный глоссарий 75 шкал; раньше разбор по странице результата без пояснений «понимал, что происходит». Предложение ведущего: компактный режим — полные пояснения только по шкалам вне 40–65T, для остальных одна строка; решение после WP8.
- Следующий шаг: merge/выкладка по сигналу владельца; 07.G5 сверка глоссария по источникам владельца; S3.5 (остальные чистые записи).
...
- Цель (запрос владельца 15.09): выдавать результаты вместе с интерпретациями целиком для сохранения и печати.
- Сделано: `CaseExportPresenter` собирает документ (шапка без ID/токенов, «Конфиденциально», базовый результат, 55 шкал, парный блок для Лазаруса, профессиональное заключение, понятный разбор — опубликованная версия с номером или черновик с пометкой, по галочкам анкета и заметка, подвал); `GET /admin/invited-case/{id}/export.pdf` (attachment, файл на сервере не остаётся) и `GET …/print` (print-CSS, `X-Robots-Tag: noindex`, кнопка печати); блок «Выгрузка» в карточке; `OwnerLabels` — русские подписи статусов сессии и режима хранения в карточке и на панели. Попутно исправлен PDF результата: колонки профиля на `inline-block` DomPDF не раскладывал (профиль вытягивался в столбик на десяток страниц) — переложены на ячейки таблицы; заголовки секций в PDF; `min-width:0` для flex-body (горизонтальная прокрутка таблицы шкал на 390px).
- Проверки и evidence: исполнитель — полный `composer test` 533 tests / 34430 assertions OK (+9 `CaseExportTest`); analyse/lint/architecture/baseline OK; браузер: печатная версия с каноническим графиком СМИЛ и 55 шкалами, парный Лазарус с совмещённым профилем; PDF 14 страниц (СМИЛ) и 11 (пара), кириллица, шапка без токенов; 390×844 без переполнения; console errors 0. Ведущий: `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден** (после merge с main).
- Долг на решение владельца: **в PDF профиль СМИЛ по-прежнему столбчатый** (состояние существующего PDF результата, переиспользовано по заданию) — расходится с PRODUCT_RULES §8; канонический профиль есть только в версии для печати (рисует браузер). Нужен серверный рендер профиля (SVG→растр) отдельным пакетом; он же нужен для .docx. Парный совмещённый профиль в PDF исключён (SVG с градиентами DomPDF не рисует). Оценка .docx: `phpoffice/phpword`, ~1–1,5 дня, после серверного рендера графика.
...
- Причина: два «Заказать заново» с разницей в 7 с (15.09 18:19) — первый запуск взял одно задание и завершился, второй погашен троттлингом (10 с); понятный разбор остался `pending` без обработчика. Ручной запуск по SSH хостинг убивает сразу.
- Сделано: лаунчер запускает воркер с `--limit` не меньше `DRAIN_LIMIT = 10`, воркер вычерпывает очередь; карточка кейса и опрос статуса при наличии `pending`-задания у кейса поднимают воркер заново (троттлинг защищает от лишних процессов).
- Проверки: launcher unit + контракты кабинета + cookie policy — OK; `composer test:fast` OK; analyse/lint OK.

### 08.B19 / 08.B20 — staging-выкладки S3.3 (`eddee4c`) и K5i (`0f32c96`)

- Этап / ветка / commit: этап 08, `codex/08-deploy-0f32c96`; deployed runtimes `eddee4c` (merge PR #112: S3.3 + G3) и `0f32c96` (merge PR #113: «Заказать заново» для готовых черновиков). Миграций нет. Между ними ошибочно повторно выложен `70cfeba` (main без изменений) из-за сбоя автоматической цепочки — без последствий.
- Сделано: артефакты `release-eddee4c.tar.gz` (SHA-256 `e7cc502b…72a7`) и `release-0f32c96.tar.gz` (`9fa122a8…bcee`) совпали после загрузки; `.env` из прежнего релиза; pre-deploy dump для каждого (gzip -t OK); `public_html`/`current` атомарно на `releases/0f32c96`.
- Проверки: основные маршруты `200`, `/test/smil` `404`.
- Rollback: `public_html` → `releases/eddee4c/public`, `current` → `releases/eddee4c` (далее `9b80e42`).

### 07.K5i — «Заказать заново» для готовых черновиков

- Этап / ветка / commit: этап 07, `codex/07-k5i-reorder-ready` (merge #113).
- Причина: владелец хотел перезаказать готовые черновики после выкладки глоссария и промпта v3, кнопка была только у неудавшихся.
- Сделано: `AiReportRepository::request(..., allowExhaustedRetry: true)` принимает и `ready` (снимок пересобирается, счётчик обнуляется); `AiReportRevisionService::seedFromContent` при существующих версиях кладёт новый текст модели следующей версией с источником «модель», если он отличается; карточка кейса: форма «Заказать заново» у обоих готовых видов с полем `kind`, перезаказывается только выбранный вид; правки специалиста и публикация не затрагиваются. Regression в `AiReportQueueTest`.
- Проверки: очередь, версии, контракт кабинета — 39 tests / 368 assertions OK; fast 419; analyse/lint OK; CI MySQL 5.7/8.0 зелёный.

### 05.S3.3 + 07.G3 — третья партия дополнительных шкал СМИЛ (психотический и психопатический спектр) и её глоссарий

- Этап / ветка / commit: этап 05/07, `codex/05-s3-3-psychotic-batch` (rebase на `main`); commits `b52ae50`, `dd94f17`, `b560601`, `839a5d8` (после rebase другие sha).
- Цель: партия из 20 записей (№61, 138–144, 146–148, 152, 153, 156–158, 170, 182, 183, 187), утверждённая владельцем 15.09; глоссарий утверждён 15.09.
- Сделано: `additional-scales-v2.json` — 55 шкал (партии 1–2 идентичны запись-в-запись), эталоны 8 наборов × 55, инварианты; `additional_scales_count` 55; глоссарий 55/55 (подтверждённые Pa1/Pa2/Pa3, Pa-O/S, Pd1/Pd2, Pd-O/S, Sc1/Sc2/Sc6; «чистые» части 6-й и 4-й; исторические шкалы без прототипа с оговоркой «не диагноз»; ошибка перевода №140 Poignancy отмечена); порог размера AI-контекста 28 → 44 тыс. знаков (факт ≈40,5 тыс.); реестр, README модуля, AUDIT_TRACEABILITY (55/113); уточнение: №146 «Чистая психопатия» вложена в 4-ю шкалу (в `smil-western-crosscheck.md` было указано Sc — исправлено там же).
- Проверки и evidence: исполнитель — targeted 86 tests / 30642 assertions OK; полный `composer test` 521 tests / 34272 assertions OK; analyse/lint/architecture/baseline OK; `smil-build-batch.php --check` OK; golden базовых без diff; браузер: 55 строк, PDF 200, 390×844 без переполнения, console errors 0. Ведущий: `bin/local-gate.sh` на Docker MySQL 5.7.44 — **пройден**.
- Следующий шаг: выкладка; S3.4 (гипомания и социальные шкалы, 20) и S3.5 (остальные чистые), затем записи с аномалиями после сверки по другому изданию.
