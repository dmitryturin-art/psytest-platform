...
- Не сделано / риски: ещё нет GitHub CI и automated browser coverage; notice не заменяет emergency resource directory и намеренно не публикует контакты.
- Следующий шаг: после возобновления — review branch, fast-forward merge/push и дождаться CI; затем отдельный package automated browser coverage.
