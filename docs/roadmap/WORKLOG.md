...
- Причина: замечание владельца 15.09 — серые кнопки без модификатора («Заказать заново», «Обновить») не находились глазами.
- Сделано: в карточке кейса и редакторе кнопки без модификатора переведены в `btn-primary` (основное действие) и `btn-outline` (второстепенное); `.btn-outline` — синяя рамка 2px и синий текст.
- Проверки: `OwnerDashboardContractTest` OK; `composer test:fast` OK; lint OK.
...
- Причина: воркер, запущенный по SSH (`nohup`, затем `setsid`), хостинг убивает через несколько минут; профессиональное заключение владельца трижды застряло в `running` и после `releaseStuck` станет `failed` с исчерпанными попытками — из кабинета его нельзя было заказать снова (R3 запрещает автоматическое оживление).
- Сделано: `AiReportRepository::request(..., allowExhaustedRetry: true)` — только из кабинета специалиста: исчерпанное failed-задание переводится в pending с `attempts = 0`, снимок входа сохраняется; кнопка в карточке кейса переименована в «Заказать заново»; страница результата посетителя поведения не меняет. Regression в `AiReportQueueTest`.
- Проверки: `AiReportQueueTest` + `OwnerDashboardContractTest` + `AiReportSnapshotTest` — 36 tests / 370 assertions OK; `composer test:fast` 419 OK; analyse/lint OK.
- Эксплуатация: воркер запускать только из веб-запроса (лаунчер) или через cron панели Beget; ручной запуск по SSH ненадёжен (зафиксировано в 08.B15).
