...
| ID | Finding | Риск | Этап | Требуемое доказательство | Статус |
|---|---|---|---|---|---|
| SMIL-ADD-01 | 200 metadata против 23 фактических шкал; R/Es/Do сомнительны | P0 для проф. продукта | 05 | 05.S1 реестр приложения (113 записей), 05.S2 транскрипция двумя независимыми проходами, 05.S3.1 первая партия: 16/113 шкал `verified` из источника, прежние 23 неподтверждённых кода выведены из расчёта; 05.S3.2 вторая партия: +19 клинических подшкал (все `verified`; №87 с пометкой об опечатке источника, подтверждённой сверкой с Hy4; №72 выведена — нормы утрачены), 05.S3.3 третья партия: +20 шкал психотического и психопатического спектра (все `verified`, аномалий сверки нет, нормы по обоим полам в источнике), 05.S3.4 четвёртая партия: +20 шкал гипоманиакального спектра и социального функционирования (все `verified`; подтверждены Ma1, Ma2 и пара Ma-Obvious/Ma-Subtle — её половины не пересекаются и в объединении дают ровно 46 пунктов 9-й базовой шкалы, а также Cn Cuadra, Ego-Sensitivity Pepper–Strong и Edwards SD по длине ключа; у №200 прототип Gough St отвергнут — 34 пункта против 19; у №177 исправлена опечатка заголовка источника с пометкой в `note`), 05.S3.5 пятая партия: +30 оставшихся записей приложения (все `verified`; подтверждены по длине Ca Caudality Williams 1952 — 36 п., что заодно вскрыло ошибку перевода названия (caudality = задние поражения мозга, не хвостатое ядро), Gough Ds-r 40 п., Harris–Lingoes Hy2 12 п. и Hy5 7 п., Gough Pr 32 п., Tydlaska–Mengel Wa 37 п., Wiener–Harmon D-Subtle 20 п. — у последней ключи №51 и №56 без пересечения дают ровно 60 пунктов 2-й базовой шкалы, и это же равенство разрешило спорное чтение сжатого кегля (160, а не 166); №162 и №205 опознаны как зеркальная пара Gough «предубеждённость — терпимость» — 29 общих пунктов, 26 в обратном направлении; три пары омонимов названий разведены через NAME_OVERRIDES — «Преступность (I)/(II)» и «Доминирование (II)»; №74 принята с нормами, совпадающими со строкой выведенной №72), итого 105/113, `additional_scales_count` = 105; вне партий остаются 8 записей: №9, 58, 72, 167 (ждут другого издания) и №175, 176, 178, 179 (шкалы одного пола, партия S3.6); сверка с западными ключами MMPI — `docs/smil-western-crosscheck.md`; инварианты `AdditionalScalesInvariantsTest` (закрытый список записей с полем `note` — №47, 50, 52, 55, 56, 74, 87, 162, 177, 205), независимый эталон `bin/smil-additional-reference.py` + `AdditionalScalesReferenceTest` (8 наборов ответов × 105 шкал), базовые 13 шкал и golden не менялись, партии 1–4 в runtime-файле не изменены запись-в-запись. Для закрытия нужна партия S3.6 (шкалы только для одного пола) | Частично (105/113) |
...
| UX-03 | Lazarus legends/touch/accessibility неудобны | P1 | 04 | 04.0D/0E: mobile-читаемость таблиц и legends, единый meter; 04.0G: aria-label на всех точках графика, тап-тултипы; 04.0H: touch hit-области 24px, контраст текстов графика ≥4.5:1 (WCAG AA) с контраст-контрактом в PairComparisonVisualTest | Закрыто |
