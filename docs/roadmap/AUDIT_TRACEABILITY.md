...
| ID | Finding | Риск | Этап | Требуемое доказательство | Статус |
|---|---|---|---|---|---|
| SMIL-ADD-01 | 200 metadata против 23 фактических шкал; R/Es/Do сомнительны | P0 для проф. продукта | 05 | 05.S1 реестр приложения (113 записей), 05.S2 транскрипция двумя независимыми проходами, 05.S3.1 первая партия: 16/113 шкал `verified` из источника, прежние 23 неподтверждённых кода выведены из расчёта; 05.S3.2 вторая партия: +20 клинических подшкал (18 `verified`, №72 и №87 — `verified-with-note` с видимой в результате и PDF пометкой «нормы требуют сверки» и текстом причины), итого 36/113, `additional_scales_count` = 36; инварианты `AdditionalScalesInvariantsTest` (закрытый список исключений для `verified-with-note`), независимый эталон `bin/smil-additional-reference.py` + `AdditionalScalesReferenceTest` (8 наборов ответов × 36 шкал), базовые 13 шкал и golden не менялись, партия 1 в runtime-файле не изменена. Для закрытия нужны остальные партии до целевого набора | Частично (36/113) |
