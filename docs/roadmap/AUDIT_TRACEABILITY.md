...
| CLIN-01 | BDI item 9 не создаёт самостоятельный crisis signal | P0 | 02 | 02.2A `16c4730`: `ClinicalSafetySignal` создаёт structured signal из валидированного item 9 при низком total; GitHub Actions `31948009328` — success. 02.2B `788e590`: approved generic notice показывается после item 9 > 0 без country/IP/GeoIP/resource reader; HTTP fixture, desktop/mobile QA и GitHub Actions [32503879209](https://github.com/dmitryturin-art/psytest-platform/actions/runs/32503879209) — success. CountryResolver и пустой registry остаются не подключёнными по D-026. Нужны automated browser cases. | В работе |
| DATA-01 | Privacy claims расходятся с plaintext/внешним AI | P0 | 02/07 | 02.4A `a14f5eb`: factual public privacy/delete copy; CI `31949538307` — success. 02.7A `73ed294`: новые sessions/activity records сохраняют IP/User-Agent как `NULL`, integration и public-copy regression пройдены. Остаются legal review, старые metadata rows и AI consent/provider boundary | В работе |
