...
| CLIN-01 | BDI item 9 не создаёт самостоятельный crisis signal | P0 | 02 | 02.2A `16c4730`: `ClinicalSafetySignal` создаёт structured signal из валидированного item 9 при низком total; GitHub Actions `31948009328` — success. 02.2B в `codex/02-bdi-crisis-notice`: approved generic notice показывается после item 9 > 0 без country/IP/GeoIP/resource reader; HTTP fixture подтверждает точный текст. CountryResolver и пустой registry остаются не подключёнными по D-026. Нужны browser cases и publication/CI. | В работе |
