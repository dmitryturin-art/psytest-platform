...
| CLIN-01 | BDI item 9 не создаёт самостоятельный crisis signal | P0 | 02 | 02.2A `16c4730`: `ClinicalSafetySignal` создаёт structured signal из валидированного item 9 при низком total; GitHub Actions `31948009328` — success. 02.3A добавляет unit-tested `CountryResolver` с manual/session/trusted-hint/unknown precedence без IP и GeoIP API. Нужны Crisis UI, ресурсы и browser cases после owner-approved текста | В работе |
