...
| ID | Finding | Риск | Этап | Требуемое доказательство | Статус |
|---|---|---|---|---|---|
| SMIL-ADD-01 | 200 metadata против 23 фактических шкал; R/Es/Do сомнительны | P0 для проф. продукта | 05 | 05.S1 реестр приложения (113 записей), 05.S2 транскрипция двумя независимыми проходами, 05.S3.1 первая партия: 16/113 шкал `verified` из источника, прежние 23 неподтверждённых кода выведены из расчёта; 05.S3.2 вторая партия: +19 клинических подшкал (все `verified`; №87 с пометкой об опечатке источника, подтверждённой сверкой с Hy4; №72 выведена — нормы утрачены), итого 35/113, `additional_scales_count` = 35; сверка с западными ключами MMPI — `docs/smil-western-crosscheck.md`; инварианты `AdditionalScalesInvariantsTest` (закрытый список исключений для `verified-with-note`), независимый эталон `bin/smil-additional-reference.py` + `AdditionalScalesReferenceTest` (8 наборов ответов × 35 шкал), базовые 13 шкал и golden не менялись, партия 1 в runtime-файле не изменена. Для закрытия нужны остальные партии до целевого набора | Частично (35/113) |
