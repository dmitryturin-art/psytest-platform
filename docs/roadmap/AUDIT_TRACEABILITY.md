...
| ID | Finding | Риск | Этап | Требуемое доказательство | Статус |
|---|---|---|---|---|---|
| SMIL-ADD-01 | 200 metadata против 23 фактических шкал; R/Es/Do сомнительны | P0 для проф. продукта | 05 | 05.S1 реестр приложения (113 записей), 05.S2 транскрипция двумя независимыми проходами, 05.S3.1 первая партия: 16/113 шкал `verified` из источника, прежние 23 неподтверждённых кода выведены из расчёта, `additional_scales_count` = 16; инварианты `AdditionalScalesInvariantsTest`, независимый эталон `bin/smil-additional-reference.py` + `AdditionalScalesReferenceTest` (8 наборов ответов), базовые 13 шкал и golden не менялись. Для закрытия нужны остальные партии до целевого набора | Частично (16/113) |
