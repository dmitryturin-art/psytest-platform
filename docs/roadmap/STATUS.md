...
- Состояние: этап 01 завершён; для этапа 02 приняты retention (180 дней / бессрочный therapist-режим) и отдельный AI-consent. 02.1, 02.2A и 02.3A подтверждены GitHub CI; 02.3B строит пустой fail-closed фундамент resource registry, а Crisis UI/text/resources остаются отдельными пакетами.
- Последний опубликованный code commit: `5942587` в `main`; GitHub Actions [31948360267](https://github.com/dmitryturin-art/psytest-platform/actions/runs/31948360267) — success, включая manual-first CountryResolver.
...
| 02 | В работе | 02.1 lifecycle, 02.2A server-side BDI signal и 02.3A CountryResolver подтверждены CI; 02.3B registry foundation проходит проверку. Crisis UI/text/resources, therapist-delete и AI consent record ещё впереди |
