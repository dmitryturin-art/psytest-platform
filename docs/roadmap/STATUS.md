...
- Этап 07, пакет 07.7: СМИЛ спрашивает возраст (решение владельца) и **проверяет его на сервере** — до этого возраст вообще не валидировался, числясь «лишним ключом». Границы 16–100 объявлены один раз и читаются и формой, и валидатором. В подсчёт возраст не входит: нормы Собчик гендерные, неизменность scoring закреплена тестом. Нижняя граница 16 требует подтверждения владельца.
