...
1. Слить стек PR #35 → #36 → #37 и выложить релиз на `test.23time.ru` по PRODUCTION_RUNBOOK (pre-deploy дамп, атомарное переключение, готовый откат на `c62f34a`).
2. Включить кабинет владельца на `test.23time.ru`: сгенерировать Argon2id hash и внести `OWNER_DASHBOARD_PASSWORD_HASH` в серверный `.env`. Код кабинета готов с 02.6, он выключен только отсутствием hash.
3. Этап 07 (по D-037): начать с WP1–WP3 — provider adapter, prompt registry на основе `docs/lazarus-ai-report-prompts.md`, structured context из уже существующей выдачи модулей. Разбор запускается бесплатно.
...
- Выбор ИИ-провайдера и модели для этапа 07 и лимит расходов на разбор.
