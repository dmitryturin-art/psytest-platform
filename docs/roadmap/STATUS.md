...
1. Внести `OPENROUTER_API_KEY` в локальный `.env` (сейчас пуст) и выбрать конкретную бесплатную модель — без этого адаптер не сделает ни одного вызова.
...
- Конкретная бесплатная модель OpenRouter и внесение `OPENROUTER_API_KEY` в `.env` — блокирует первый вызов (провайдер выбран, D-039).
