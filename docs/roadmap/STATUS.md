...
1. Владелец читает и одобряет шесть промптов в `prompts/` — до одобрения они `draft` и в боевой поток не идут. Отдельно нужен взгляд на понятный вариант СМИЛ: он написан без участия владельца.
...
- Выбор ИИ-провайдера и модели для этапа 07 и лимит расходов на один разбор — блокирует WP1.
- Промпт для СМИЛ (у владельца готов, передаст к соответствующему пакету) — блокирует нагрузку и промпты СМИЛ. Первый релиз ИИ-разборов: только СМИЛ и Лазарус.
