...
0.4. **04.0E — polish парного результата и PDF.** В работе: meter совпадения переводится на тот же компонент шкалы, раскрытие подробной таблицы становится самостоятельной заметной кнопкой. Для PDF используется отдельная компактная таблица на A4 landscape, без web-only `<details>` и длинных заголовков. Требуются CI и staging visual acceptance; scoring не меняется.
