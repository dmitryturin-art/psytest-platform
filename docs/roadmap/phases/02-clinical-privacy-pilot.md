...
3. **Страна и кризисные ресурсы.** Не входят в этот safety-flow и не имеют code/schema-задела. Их можно обсуждать только отдельным новым решением владельца.
...
8. **Закрытый бесплатный пилот.** 02.8A: подготовлен [операционный runbook](../PILOT_RUNBOOK.md) для двух небольших волн с anonymized issue log, личным support path владельца, smoke-check desktop/mobile и немедленной остановкой при P0. Оплата, AI, аккаунты и public production явно исключены. Фактический запуск ждёт отдельного staging и выполнения checklist.
...
11. **Technical metadata minimization.** 02.7A `73ed294`: новые test sessions и activity records больше не собирают IP/User-Agent; legacy-колонки остаются nullable для совместимости. Старые значения массово не удаляются без отдельного решения. 02.7C: по решению владельца (D-035) колонки `ip_address`/`user_agent` удалены из `test_sessions` и `activity_log` миграцией `20260825120000` вместе со старыми значениями; код больше не передаёт метаданные, `MigratedSchemaTest` и `SessionDataMinimizationTest` закрепляют отсутствие колонок.
