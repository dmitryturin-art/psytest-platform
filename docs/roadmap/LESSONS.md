...
- Защита: generic safety notice без country/IP/GeoIP и без непроверенных контактов.
- Статус: действует.
...
## L-015 — Незаякорённые exclude-паттерны в сборке артефакта молча выкидывают трекаемые ассеты

- Наблюдение: при сборке релиза `4775dc4`/`1ccd53f` rsync-паттерн `smil-*.png` (без якоря `/`) был задуман для dev-скриншотов в корне, но совпал по имени с трекаемым ассетом `public/images/smil-profile-bg.png` и выкинул его из артефакта. Классический график СМИЛ остался без фоновой сетки (битая картинка 404); владелец заметил визуально.
- Риск: тихая деградация прод-ассетов при каждом деплое; автотесты этого не видят, т.к. в репозитории файл есть.
- Правило: в сборке артефакта все exclude — только с якорем корня (`/pattern`); после сборки сверять `git ls-files public` с содержимым артефакта.
- Защита: `bin/build-release.sh` (якорные exclude + верификация полноты public/ и отказ при грязном дереве); `public/images/smil-profile-bg.png` добавлен в static-список `bin/check-architecture.php`.
- Статус: закрыто 25.08.2026.
