...
Rollback текущего релиза: атомарно направить `public_html` на `releases/release-c8e2b28/public` и `current` на `releases/release-c8e2b28`; pre-deploy dump и прежние releases сохранены в `backups/` и `releases/`. Следующий шаг — K2 (клиенты/назначения) или короткий owner-pilot; production go-live отдельно.
