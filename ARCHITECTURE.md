...
Дополнительные шкалы СМИЛ живут отдельно от замороженных базовых 13: `modules/smil/additional-scales-v2.json` генерируется `bin/smil-build-batch.php` из транскрипции приложения Собчик (`docs/smil-additional-scales-transcription.json`), а `AdditionalScalesCalculator` считает по нему raw и T по нормам пола, без клинических текстов. В runtime попадают только записи со статусом `verified`; на 15.09.2026 это 16 шкал партии 05.S3.1, прежние 23 неподтверждённых кода выведены из расчёта. Ожидаемые значения проверяются независимым эталоном `bin/smil-additional-reference.py` → `tests/fixtures/smil-additional-batch1-reference.json`. Рядом лежит `modules/smil/additional-scales-glossary.json` — короткие пояснения к этим шкалам и правило уровней для внешнего ИИ (07.G1): пересказ своими словами, без клинических текстов платформы и без цитат из источника. `SmilModule::aiReportContext()` добавляет в контекст блок `additional_scales_glossary` только по фактически переданным шкалам и блок `levels`; шкала без записи уходит без пояснения и контекст не ломает. Насколько подробен глоссарий, решает уже не модуль, а `core/Ai/SmilGlossaryCompactor.php` (07.G6): он применяется поверх готовой нагрузки в `AiReportContextBuilder` и `PromptFixtureContext` по настройке `smil_glossary_mode`, в компактном режиме оставляет у шкал с T в диапазоне 40–65 только `meaning`, дописывает в `levels.principles` фразу об этом и всегда проставляет `glossary_mode`, так что по `context_snapshot` видно, каким глоссарием сделан разбор. Модуль о настройках кабинета по-прежнему не знает.
...
`therapist_clients` — карточка клиента специалиста: подпись владельца, необязательная заметка и необязательный email. Email — единственный контакт, который платформа хранит, и только по решению специалиста ради одного уведомления (D-054): `core/ClientReportNotifier.php` по маршруту POST `/admin/invited-case/{sessionId}/reports/notify` отправляет короткое письмо «Ваш разбор готов» — без текста разбора и без ссылки на результат — при условии опубликованной clear-ревизии и заполненного адреса, не чаще раза в 10 минут на разбор (`ai_reports.client_notified_at`); сбой отправки логируется без адреса и возвращает `false`. `core/TherapistClientService.php` создаёт и изменяет карточку, собирает её назначения (`test_invites.client_id`) со статусами и историю завершённых прохождений, а при удалении одной транзакцией проводит каждую связанную session через `SessionLifecycleService` и удаляет карточку, унося приглашения каскадом. Приглашение без карточки остаётся полноправным: связь необязательна. Полный кабинет с отчётами относится к этапу 07. Фактические границы — в [DATA_MAP_CURRENT.md](docs/roadmap/DATA_MAP_CURRENT.md), policy — в [RETENTION_POLICY.md](docs/roadmap/RETENTION_POLICY.md).
