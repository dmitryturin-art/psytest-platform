...
PsyTest Platform — веб-платформа для проведения и обработки психологических тестирований. Поддерживает пять методик (СМИЛ, BDI, HADS, BAI и Lazarus) с автоматическим расчётом шкал, валидацией, базовым результатом и генерацией PDF-отчётов.
...
│   └── lazarus/               # Опросник супружеской удовлетворённости (16 пунктов)
...
BaseTestModule (абстрактный, ~280 строк)
        ▲
┌───────┴────────┬────────────────┬─────────────┐
SmilModule       BeckDepression   HadsModule   BeckAnxiety   LazarusModule
```
...
### Crisis resource registry — `crisis_resources`

Инкрементальная migration создаёт пустой реестр проверяемых кризисных ресурсов, не связанный с сессиями и не содержащий персональных данных. Для записи предусмотрены страна (или `NULL` для международного fallback), язык, тип, отображаемое имя, контакт или URL, официальный источник, дата и автор проверки. По умолчанию `active = 0`, поэтому новая запись не может стать публичной сама. Сейчас нет ни seed-контактов, ни reader/UI, ни правила просроченности: их добавление требует утверждённого владельцем набора ресурсов и срока актуальности.
